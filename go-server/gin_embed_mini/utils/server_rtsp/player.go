...
func (player *Player) QueueRTP(pack *RTPPack) *Player {
	...
	if pack == nil {
		logs.Debug("player queue enter nil pack, drop it")
		return player
	}
	...
	if oldLen := len(player.queue); player.queueLimit > 0 && oldLen > player.queueLimit {
		...
		if player.debugLogEnable {
			...
			logs.Debug("Player %s, QueueRTP, exceeds limit(%d), drop %d old packets, current queue.len=%d\n", player.String(), player.queueLimit, oldLen-len, len)
		}
	}
	player.cond.Signal()
...
func (player *Player) Start() {
	...
	for !player.Stoped {
		...
		if pack == nil {
			if !player.Stoped {
				logs.Debug("player not stoped, but queue take out nil pack")
			}
			continue
		}
		...
		if player.debugLogEnable && elapsed >= 30*time.Second {
			logs.Debug("Player %s, Send a package.type:%d, queue.len=%d\n", player.String(), pack.Type, queueLen)
			timer = time.Now()
		}
	}
...
func (player *Player) Pause(paused bool) {
	if paused {
		logs.Debug("Player %s, Pause\n", player.String())
	} else {
		logs.Debug("Player %s, Play\n", player.String())
	}
	player.cond.L.Lock()
