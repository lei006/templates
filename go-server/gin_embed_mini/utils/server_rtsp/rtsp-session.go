...
func (session *Session) Start() {
	...
	for !session.Stoped {
		...
		if buf1[0] == 0x24 { //rtp data
			if _, err := io.ReadFull(session.connRW, buf1); err != nil {
				session.Server.logger.Debug(err.Error())
				return
			}
			...
			case session.aRTPChannel:
				...
				if elapsed >= 30*time.Second {
					//logs.Debug("Recv an audio RTP package")
					timer = time.Now()
				}
			...
			case session.vRTPChannel:
				...
				elapsed := time.Now().Sub(timer)
				if elapsed >= 30*time.Second {
					//logs.Debug("Recv an video RTP package")
					timer = time.Now()
				}
			...
			default:
				session.Server.logger.Debug("unknow rtp pack type, %v", pack.Type)
				continue
			}
			if pack == nil {
				session.Server.logger.Debug("session tcp got nil rtp pack")
				continue
			}
					...
					if len(line) == 0 {
						...
						if contentLen > 0 {
							...
							if n, err := io.ReadFull(session.connRW, bodyBuf); err != nil {
								...
								session.Server.logger.Debug("read rtsp request body failed, expect size[%d], got size[%d]", contentLen, n)
								return
							}
...
func (session *Session) handleRequest(req *Request) {
	...
	defer func() {
		if p := recover(); p != nil {
			session.Server.logger.Debug("handleRequest err ocurs:%v", p)
			res.StatusCode = 500
		...
		if res.StatusCode != 200 && res.StatusCode != 401 {
			session.Server.logger.Debug("Response request error[%d]. stop session.", res.StatusCode)
			session.Stop()
		}
	}()
		...
		if session.authorizationEnable {
			...
			if authLine != "" {
				...
				if err == nil {
					...
					session.Server.logger.Debug("%v", err)
				}
			}
			if authFailed {
				res.StatusCode = 401
	...
	case "ANNOUNCE":
		...
		if ok {
			...
			//session.Server.logger.Debug("audio codec:", session.ACodec)
		}
		sdp, ok = session.SDPMap["video"]
		if ok {
			session.VControl = sdp.Control
			session.VCodec = sdp.Codec
			//session.Server.logger.Debug("video codec:", session.VCodec)
		}
		addPusher := false
		if session.closeOld {
			r, _ := session.Server.TryAttachToPusher(session)
			if r < -1 {
				session.Server.logger.Debug("reject pusher.")
				res.StatusCode = 406
				res.Status = "Not Acceptable"
			} else if r == 0 {
				addPusher = true
			} else {
				session.Server.logger.Debug("Attached to old pusher")
				// 尝试发给客户端ANNOUCE
	...
	case "SETUP":
		...
		if tcpMatchs := mtcp.FindStringSubmatch(ts); tcpMatchs != nil {
			...
			if setupPath == aPath || aPath != "" && strings.LastIndex(setupPath, aPath) == len(setupPath)-len(aPath) {
				...
				session.Server.logger.Debug("SETUP [TCP] got UnKown control:%s", setupPath)
			}
			//session.Server.logger.Debug("Parse SETUP req.TRANSPORT:TCP.Session.Type:%d,control:%s, AControl:%s,VControl:%s", session.Type, setupPath, aPath, vPath)
			...
			//session.Server.logger.Debug("Parse SETUP req.TRANSPORT:UDP.Session.Type:%d,control:%s, AControl:%s,VControl:%s", session.Type, setupPath, aPath, vPath)
			if setupPath == aPath || aPath != "" && strings.LastIndex(setupPath, aPath) == len(setupPath)-len(aPath) {
				...
				session.Server.logger.Debug("SETUP [UDP] got UnKown control:%s", setupPath)
			}
		}
