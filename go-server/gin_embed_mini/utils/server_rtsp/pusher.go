...
type Pusher struct {
	...
	*RTSPClient
	...
	h264Parser *packet.Parser
	//g711Parser *g711.Parser
	...
	//packetCallbacks []PacketCallback //更新回调列表
	packetCallbackHandle PacketCallback
}

func (pusher *Pusher) SetPacketCallback(cb PacketCallback) {
	//pusher.packetCallbacks = append(pusher.packetCallbacks, cb)
	pusher.packetCallbackHandle = cb
}

func (pusher *Pusher) CallbackPacket(packet *packet.H264Packet) {

	if pusher.packetCallbackHandle != nil {
		pusher.packetCallbackHandle(packet)
	}
	/*
		for _, val := range pusher.packetCallbacks {
			val(packet)
		}
	*/
}
...
func NewPusher(session *Session) (*Pusher, error) {
	...
	pusher.PubID = strings.Replace(pub_id, ".sdp", "", -1)
...
func (pusher *Pusher) bindSession(session *Session) error {
	...
	pusher.h264Parser = packet.NewParser()
	//pusher.g711Parser = &g711.Parser{}

	session.RTPHandles = append(session.RTPHandles, func(pack *RTPPack) {
		if session != pusher.Session {
			logs.Debug("Session recv rtp to pusher.but pusher got a new session[%v].", pusher.Session.ID)
			return
		}
		pusher.QueueRTP(pack)
	})
	session.StopHandles = append(session.StopHandles, func() {
		if session != pusher.Session {
			logs.Debug("Session stop to release pusher.but pusher got a new session[%v].", pusher.Session.ID)
			return
		}
		pusher.ClearPlayer()
		pusher.Server().RemovePusher(pusher)
	...
	if pack.Type == RTP_TYPE_VIDEO {
		...
		if err == nil && newPack != nil {
			...
			if len(pusher.SDPMap["video"].SpropParameterSets) >= 2 {
				...
				logs.Warn("SpropParameterSets size < 2")
			}
			newPack.FatherId = pusher.PubID
			pusher.CallbackPacket(newPack)
		}
	}
	if pack.Type == RTP_TYPE_AUDIO {
		/*
			if len(data) > 12 {
				packet := datapacket.MakeAlawPacket(pack.Buffer.Bytes()[12:])
				packet.FatherId = pusher.PubID
				pusher.Server().CallbackPacket(packet)
				//pusher.publisher.PublishPacket(packet)
			}
		*/
	}
...
func (pusher *Pusher) Start() {
	...
	for !pusher.Stoped() {
		...
		if pack == nil {
			if !pusher.Stoped() {
				logs.Debug("pusher not stoped, but queue take out nil pack")
			}
			continue
		}
...
func (pusher *Pusher) AddPlayer(player *Player) *Pusher {
	...
	if _, ok := pusher.players[player.ID]; !ok {
		...
		go player.Start()
		logs.Debug("%v start, now player size[%d]", player, len(pusher.players))
	}
	pusher.playersLock.Unlock()
	return pusher
}
...
func (pusher *Pusher) RemovePlayer(player *Player) *Pusher {
	...
	logs.Debug("%v end, now player size[%d]", player, len(pusher.players))
	pusher.playersLock.Unlock()
	return pusher
}
