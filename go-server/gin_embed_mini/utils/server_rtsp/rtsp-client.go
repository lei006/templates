	...
	for _, media := range _sdp.Media {
		...
		case "video":
			...
			if client.TransType == TRANS_TYPE_TCP {
				...
				if err != nil {
					client.Server.logger.Debug("Setup video err.%v", err)
					return err
				}
			...
			client.Server.logger.Debug("Parse DESCRIBE response, VIDEO VControl:%s, VCode:%s, url:%s,Session:%s,vRTPChannel:%d,vRTPControlChannel:%d", client.VControl, client.VCodec, _url, session, client.vRTPChannel, client.vRTPControlChannel)
		...
		case "audio":
			...
			if client.TransType == TRANS_TYPE_TCP {
				...
				if err != nil {
					client.Server.logger.Debug("Setup audio err.%v", err)
					return err
				}
			...
			client.Server.logger.Debug("Parse DESCRIBE response, AUDIO AControl:%s, ACodec:%s, url:%s,Session:%s, aRTPChannel:%d,aRTPControlChannel:%d", client.AControl, client.ACodec, _url, session, client.aRTPChannel, client.aRTPControlChannel)
			resp, err = client.RequestWithPath("SETUP", _url, headers, true)
			if err != nil {
				return err
			}
...
func (client *RTSPClient) startStream() {
	...
	for !client.Stoped {
		...
		if err != nil {
			if !client.Stoped {
				client.Server.logger.Debug("client.connRW.ReadByte err:%v", err)
			}
			return
		}
		switch b {
		case 0x24: // rtp
			...
			if err != nil {
				...
				if !client.Stoped {
					client.Server.logger.Debug("io.ReadFull err:%v", err)
				}
				return
			}
			...
			default:
				client.Server.logger.Debug("unknow rtp pack type, channel:%v", channel)
				continue
			}
			if pack == nil {
				client.Server.logger.Debug("session tcp got nil rtp pack")
				continue
			}
			...
			if client.debugLogEnable {
				...
				if rtp != nil {
					...
					if client.lastRtpSN != 0 && client.lastRtpSN+1 != rtpSN {
						client.Server.logger.Debug("%s, %d packets lost, current SN=%d, last SN=%d\n", client.String(), rtpSN-client.lastRtpSN, rtpSN, client.lastRtpSN)
					}
				...
				if elapsed >= 30*time.Second {
					client.Server.logger.Debug("%v read rtp frame.", client)
					loggerTime = time.Now()
				}
			}
		...
		default: // rtsp
			...
			for !client.Stoped {
				...
				if err != nil {
					if !client.Stoped {
						client.Server.logger.Debug("client.connRW.ReadLine err:%v", err)
					}
					return
				}
				if len(line) == 0 {
				...
				if strings.Index(s, "Content-Length:") == 0 {
					...
					if err != nil {
						if !client.Stoped {
							client.Server.logger.Debug("strconv.Atoi err:%v, str:%v", err, splits[1])
						}
						return
					}
				}
...
func (client *RTSPClient) RequestWithPath(method string, path string, headers map[string]string, needResp bool) (resp *Response, err error) {
	...
	s := builder.String()
	client.Server.logger.Debug("[OUT]>>>\n%s", s)
